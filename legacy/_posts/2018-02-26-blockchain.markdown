...
1 参赛作品名称: python 语言比原智能合约编译器
参赛作品简介: 使用 python 语言 进行编写 比原智能合约，编译成 .wasm 由比原虚拟机执行

2 参赛作品名称: 基于比原的中心化交易所
参赛作品简介: 比原天然支持多资产的特性，使得 多资产交易 成为一个优秀的落地场景。
由于交易所需要应对高并发请求、高频实时交易，为了更好的用户体验，选择搭建中心化交易所。

3 参赛作品名称: 基于比原的电商平台
参赛作品简介: 由于目前的电商平台都是 中心化的，数据都集中存储在 电商数据中心，存在 电商篡改数据/被黑风险。
该项目旨在在原本传统电商服务的基础上，将 交易相关 数据上链，用户交易成功后可使用 tx hash 查询。永久记录、不可篡改。

4 参赛作品名称: 比原安卓离线签名
参赛作品简介: 使用 安卓手机 进行私钥管理和交易签名，节点不需要存储私钥，防止节点被黑私钥丢失，发起交易更加安全。

5 参赛作品名称: 基于比原的合同等文档上链平台
参赛作品简介: 文档上链 平台提供 一个文档存证、储存系统。平台本身不明文保存文件，而保存 经私钥加密后的 文件。
比如合同经双方签署甚至公证后，可用双方的私钥分别进行加密后，上传到 文件存储服务器上。平台再将 文件上链存储。用户将来可以从链上读取加密文件，使用双方公钥进行两步解密。
从而证明了文件由双方都进行了签名，文件的签署得到了双方的同意。
由于 区块链 具有永久 存储不可篡改的 特性。从而避免了 合同文本本身的损毁/丢失/被盗。也防止了 篡改和伪造。
...
<!-- 觉得将来靠区块链赚的比较多的是赌博行业，甚至可以混币洗钱什么的
 -->
